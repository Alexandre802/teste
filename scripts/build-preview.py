...
.lgBtn{{display:flex;width:100%;align-items:center;justify-content:center;gap:.75rem;border-radius:999px;padding:.9rem 1.5rem;font-weight:800;transition:transform .2s,background .2s,color .2s;margin-bottom:.75rem}}
.lgBtn:hover:not(:disabled){{transform:translateY(-2px)}}
.lgBtn:disabled{{opacity:.45;cursor:not-allowed}}
.lgBtn svg{{width:1.25rem;height:1.25rem;flex:none}}
.lgGoogle{{background:#fff;color:var(--cocoa)}}
.lgFb{{background:#1877F2;color:#fff}}
.lgMail{{border:2px solid rgb(255 255 255/.6);color:#fff}}
.lgMail:hover:not(:disabled){{background:#fff;color:var(--cocoa)}}
.lgGuest{{background:rgb(255 255 255/.18);color:#fff;box-shadow:inset 0 0 0 1px rgb(255 255 255/.4)}}
.lgGuest:hover{{background:rgb(255 255 255/.28)}}
.lgOu{{display:flex;align-items:center;gap:.75rem;margin:.75rem 0;font-size:.72rem;font-weight:800;letter-spacing:.2em;text-transform:uppercase;color:rgb(255 255 255/.7)}}
.lgOu span{{flex:1;height:1px;background:rgb(255 255 255/.3)}}
.lgIn{{width:100%;border-radius:1rem;border:1px solid rgb(255 255 255/.3);background:rgb(255 255 255/.16);padding:.75rem 1rem;color:#fff;margin-bottom:.75rem}}
.lgIn::placeholder{{color:rgb(255 255 255/.6)}}
.lgCod{{text-align:center;font-size:1.5rem;font-weight:800;letter-spacing:.4em}}
.lgErro{{border-radius:1rem;border:1px solid rgb(255 255 255/.5);background:rgb(168 56 10/.45);padding:.75rem 1rem;margin-top:.75rem}}
.lgErro b{{display:block;font-size:.875rem}}
.lgErro small{{display:block;margin-top:.25rem;font-size:.75rem;color:rgb(255 255 255/.85)}}
.lgTrava{{display:flex;gap:.75rem;border-radius:1rem;border:1px solid rgb(255 255 255/.45);background:rgb(255 255 255/.16);padding:1rem;margin-bottom:1rem}}
.lgTrava svg{{width:1.25rem;height:1.25rem;flex:none;margin-top:.15rem}}
.lgLink{{display:block;width:100%;margin-top:.5rem;font-size:.875rem;font-weight:700;color:rgb(255 255 255/.85);text-decoration:underline;text-underline-offset:4px}}
.lgLink:hover{{color:#fff}}
...
function renderCart(){{
  ...
  document.querySelector('#dlgCart h2').textContent = 'Sua sacola';
  if (!cart.length) {{
  ...
  document.getElementById('irLogin')?.addEventListener('click', abrirLogin);
}}

/* ── identificação antes do pagamento ─────────────────────────────────
   Entrar é opcional: o modo convidado fica sempre disponível, inclusive
   com o login bloqueado. O bloqueio por tentativas é trava de INTERFACE —
   vive no navegador e qualquer um zera limpando os dados do site. Contém
   erro de digitação, não ataque; proteção real é limite por IP no servidor. */
const MAX_TENTATIVAS = 5, BLOQUEIO_MIN = 15;
const CHAVE_AUTH = 'mfh-auth-preview-v1';
let auth = {{ tentativas: 0, bloqueadoAte: null }};
try {{ auth = JSON.parse(localStorage.getItem(CHAVE_AUTH)) || auth; }} catch {{}}
let cliente = null;
try {{ cliente = JSON.parse(localStorage.getItem('mfh-cliente-preview')) || null; }} catch {{}}
const salvaAuth = () => {{ try {{ localStorage.setItem(CHAVE_AUTH, JSON.stringify(auth)); }} catch {{}} }};
const salvaCliente = () => {{ try {{ localStorage.setItem('mfh-cliente-preview', JSON.stringify(cliente)); }} catch {{}} }};
const bloqueado = () => {{
  if (!auth.bloqueadoAte) return false;
  if (Date.now() >= auth.bloqueadoAte) {{ auth = {{tentativas:0, bloqueadoAte:null}}; salvaAuth(); return false; }}
  return true;
}};
const espera = () => {{
  const s = Math.ceil((auth.bloqueadoAte - Date.now())/1000);
  if (s >= 60) {{ const m = Math.max(1, Math.round(s/60)); return `${{m}} ${{m===1?'minuto':'minutos'}}`; }}
  return `${{s}} segundos`;
}};
const falhar = (msg) => {{
  auth.tentativas++;
  if (auth.tentativas >= MAX_TENTATIVAS) auth.bloqueadoAte = Date.now() + BLOQUEIO_MIN*60000;
  salvaAuth(); telaLogin(estado, msg);
}};
const entrar = (nome, via) => {{
  auth = {{tentativas:0, bloqueadoAte:null}}; salvaAuth();
  cliente = {{nome, via}}; salvaCliente();
  estado = {{tela:'escolha'}};
  renderCart();
}};

const SVG_G = `<svg viewBox="0 0 48 48"><path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.9 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.05 6.05 29.3 4 24 4 12.95 4 4 12.95 4 24s8.95 20 20 20 20-8.95 20-20c0-1.3-.14-2.4-.4-3.5z"/><path fill="#FF3D00" d="m6.3 14.7 6.6 4.8C14.7 15.1 19 12 24 12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.05 6.05 29.3 4 24 4 16.3 4 9.7 8.35 6.3 14.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.85-1.95 13.4-5.2l-6.2-5.2C29.2 35.1 26.7 36 24 36c-5.25 0-9.7-3.1-11.3-7.9l-6.55 5.05C9.5 39.6 16.2 44 24 44z"/><path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.75 2.15-2.15 4-3.9 5.3l6.2 5.2C37.2 39.9 44 35 44 24c0-1.3-.14-2.4-.4-3.5z"/></svg>`;
const SVG_F = `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.96h-1.51c-1.49 0-1.96.93-1.96 1.89v2.26h3.33l-.53 3.49h-2.8V24C19.61 23.1 24 18.1 24 12.07z"/></svg>`;
const SVG_M = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>`;
const SVG_L = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`;

let estado = {{tela:'escolha'}};
const abrirLogin = () => {{ estado = {{tela:'escolha'}}; telaLogin(estado); }};

function telaLogin(st, erro){{
  estado = st;
  const body = document.getElementById('cBody'), foot = document.getElementById('cFoot');
  const trava = bloqueado();
  const restam = Math.max(0, MAX_TENTATIVAS - auth.tentativas);
  let html = `<p style="line-height:1.7;color:rgb(255 255 255/.9);margin-bottom:1rem">Falta pouco. Entre para a casa lembrar do seu pedido na próxima, ou siga como convidado — o pedido sai igual dos dois jeitos.</p>`;

  if (trava) html += `<div class="lgTrava">${{SVG_L}}<div><b>Login bloqueado</b><small>Foram ${{MAX_TENTATIVAS}} tentativas sem sucesso. O login volta em <strong>${{espera()}}</strong>. Você pode seguir agora como convidado.</small></div></div>`;
  else html += `<p class="aviso" style="margin:0 0 1rem">Preview sem servidor: o código de e-mail é gerado aqui mesmo e aparece na tela. Google e Facebook simulam a entrada. O bloqueio por ${{MAX_TENTATIVAS}} tentativas funciona de verdade — erre o código para ver.</p>`;

  const d = trava ? 'disabled' : '';
  if (st.tela === 'escolha') {{
    html += `<button class="lgBtn lgGoogle" data-lg="google" ${{d}}>${{SVG_G}}Continuar com Google</button>
      <button class="lgBtn lgFb" data-lg="facebook" ${{d}}>${{SVG_F}}Continuar com Facebook</button>
      <button class="lgBtn lgMail" data-lg="email" ${{d}}>${{SVG_M}}Entrar com e-mail</button>
      <div class="lgOu"><span></span>ou<span></span></div>
      <button class="lgBtn lgGuest" data-lg="convidado">Continuar como convidado</button>
      <p style="text-align:center;font-size:.75rem;line-height:1.6;color:rgb(255 255 255/.75)">Como convidado o pedido segue normalmente. Entrar só serve para a casa lembrar de você no próximo.</p>`;
  }} else if (st.tela === 'email') {{
    html += `<label class="lgLink" style="text-decoration:none;text-align:left" for="lgEmail">Seu e-mail</label>
      <input id="lgEmail" class="lgIn" type="email" inputmode="email" placeholder="voce@email.com" value="${{st.email||''}}" ${{d}}>
      <button class="lgBtn lgGuest" data-lg="enviar" ${{d}}>Enviar código</button>
      <button class="lgLink" data-lg="voltar">Voltar</button>`;
  }} else if (st.tela === 'codigo') {{
    html += `<p class="aviso" style="margin:0 0 .75rem">Nenhum e-mail foi enviado — não há servidor no preview. Use o código <strong>${{st.codigo}}</strong>.</p>
      <label class="lgLink" style="text-decoration:none;text-align:left" for="lgCod">Código de 6 dígitos</label>
      <input id="lgCod" class="lgIn lgCod" inputmode="numeric" maxlength="6" placeholder="000000" ${{d}}>
      <button class="lgBtn lgGuest" data-lg="verificar" ${{d}}>Entrar</button>
      <button class="lgLink" data-lg="voltar">Usar outro e-mail</button>`;
  }} else {{
    html += `<p style="line-height:1.7;color:rgb(255 255 255/.9);margin-bottom:.75rem">Só precisamos de um nome para a casa chamar no balcão.</p>
      <input id="lgNome" class="lgIn" placeholder="Seu nome" aria-label="Seu nome">
      <button class="lgBtn lgGuest" data-lg="salvarConvidado">Continuar como convidado</button>
      <button class="lgLink" data-lg="voltar">Voltar</button>`;
  }}

  if (erro) html += `<div class="lgErro" role="alert"><b>${{erro}}</b>${{trava?'':`<small>${{restam===1?'Resta 1 tentativa antes do login bloquear por '+BLOQUEIO_MIN+' minutos.':'Restam '+restam+' tentativas antes do login bloquear.'}}</small>`}}</div>`;
  if (st.tela !== 'convidado' && st.tela !== 'escolha') html += `<button class="lgLink" data-lg="convidado">Prefiro continuar como convidado</button>`;

  document.querySelector('#dlgCart h2').textContent = 'Identifique-se';
  body.innerHTML = html;
  foot.innerHTML = `<div class="tot"><span>Subtotal</span><b>${{BRL(total())}}</b></div><button class="lgLink" data-lg="sacola">Voltar para a sacola</button>`;
}}

document.getElementById('dlgCart').addEventListener('click', e => {{
  const b = e.target.closest('[data-lg]'); if (!b) return;
  const acao = b.dataset.lg;
  if (acao === 'sacola') return renderCart();
  if (acao === 'voltar') return telaLogin({{tela:'escolha'}});
  if (acao === 'convidado') return telaLogin({{tela:'convidado'}});
  if (acao === 'salvarConvidado') return entrar(document.getElementById('lgNome').value.trim() || 'Convidado', 'convidado');
  if (acao === 'google' || acao === 'facebook') {{
    b.textContent = 'Entrando…';
    return setTimeout(() => entrar(cliente?.nome || 'Cliente', acao === 'google' ? 'Google' : 'Facebook'), 700);
  }}
  if (acao === 'email') return telaLogin({{tela:'email'}});
  if (acao === 'enviar') {{
    const email = document.getElementById('lgEmail').value.trim();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{{2,}}$/.test(email)) return telaLogin({{tela:'email', email}}, 'E-mail inválido.');
    const codigo = String(Math.floor(100000 + Math.random()*900000));
    return telaLogin({{tela:'codigo', email, codigo}});
  }}
  if (acao === 'verificar') {{
    const digitado = document.getElementById('lgCod').value.replace(/\D/g,'');
    if (digitado === estado.codigo) {{
      const nome = estado.email.split('@')[0].replace(/[._-]+/g,' ').replace(/\b\w/g, c=>c.toUpperCase());
      return entrar(nome, 'e-mail');
    }}
    return falhar('Código incorreto.');
  }}
}});
