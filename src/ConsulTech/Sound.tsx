...
/*
 * Volume presets come from `mix.ts`, which states them in dB below the
 * narration. SFX occupy the -12 … -18 dB band, music -24 … -30 dB.
 */
const V = {
...
const CUES: Cue[] = [
	...
	{at: S.askMe + 3, name: 'shimmer', volume: SFX.shimmer},
	...
	{at: S.scattered + 150, name: 'glitch', volume: SFX.sweep},
	...
	{at: S.worse + 2, name: 'glitch', volume: SFX.low},

	/* ---------------------------------------------------------------- *
	 * 05 — the loss: clock, headline, collapsing chart
	 * ---------------------------------------------------------------- */
	{at: S.loss, name: 'whoosh-stereo', volume: SFX.low},
	{at: S.loss + 2, name: 'low-whoosh', volume: SFX.low},
	{at: S.loss + 10, name: 'text-whoosh', volume: V.textIn},
	{at: S.loss + 20, name: 'hit-soft', volume: SFX.card},
	{at: S.loss + 28, name: 'digital-sweep', volume: SFX.sweep},
	...series(S.loss + 40, 5, 14, 'ui-click', 0.14),

	/* ---------------------------------------------------------------- *
	 * 06 — the brand assembles. The hero moment of the whole ad.
	 * ---------------------------------------------------------------- */
	{at: S.birth - 50, name: 'rise-clean', volume: SFX.low},
	{at: S.birth, name: 'impact-clean', volume: SFX.hero},
	{at: S.birth + 1, name: 'impact-sub', volume: SFX.hero},
	...
	{at: S.platform + 78, name: 'hit-soft', volume: SFX.textHit},
	...series(S.platform + 82, 6, 6, 'beep', 0.17),

	/* ---------------------------------------------------------------- *
	 * 08 — the four categories, one per spoken word
	 * ---------------------------------------------------------------- */
	{at: S.categories, name: 'whoosh-soft', volume: SFX.card},
	...BEAT.categories.map((b) => ({
		at: S.categories + b,
		name: 'pop',
		volume: V.card,
	})),
	{at: S.categories + 124, name: 'digital-sweep', volume: SFX.card},

	/* 09 — "em segundos" */
	{at: S.seconds, name: 'whoosh-stereo', volume: SFX.low},
	{at: S.seconds + 1, name: 'hit-soft', volume: V.textHit},

	/* ---------------------------------------------------------------- *
	 * 10 — the service rows, each with its own live readout
	 * ---------------------------------------------------------------- */
	{at: S.services, name: 'whoosh-soft', volume: SFX.sweep},
	/* every row: a swipe as it travels, a pop as it lands */
	...BEAT.services.flatMap((b) => [
		{at: S.services + b, name: 'swipe', volume: SFX.card},
		{at: S.services + b + 5, name: 'pop', volume: SFX.card},
	]),
	...
	{at: S.services + BEAT.services[2] + 18, name: 'stamp', volume: SFX.textHit},
	/* row 4 — the alert bars */
	...series(S.services + BEAT.services[3] + 16, 3, 5, 'ui-click', 0.16),
	/* row 5 — the sparkline drawing */
	{at: S.services + BEAT.services[4] + 12, name: 'digital-sweep', volume: SFX.sweep},

	/* ---------------------------------------------------------------- *
	 * 11 — everything collapses into the mark
	 * ---------------------------------------------------------------- */
	{at: S.onePlace, name: 'low-whoosh', volume: SFX.low},
	{at: S.onePlace + 30, name: 'impact-sub', volume: SFX.hero},
	 ...
	 * ---------------------------------------------------------------- */
	{at: S.noMiddleman, name: 'whoosh-soft', volume: SFX.sweep},
	...[4, 21, 38].map((d) => ({
		at: S.noMiddleman + d,
		name: 'text-whoosh',
		volume: SFX.textIn,
	 ...
	 * ---------------------------------------------------------------- */
	{at: S.cta - 40, name: 'rise-clean', volume: SFX.transition},
	{at: S.cta, name: 'whoosh-stereo', volume: V.transition},
	{at: S.cta + 2, name: 'text-whoosh', volume: SFX.sweep},
	{at: S.cta + 46, name: 'shimmer', volume: SFX.low},
	{at: S.cta + 56, name: 'impact-clean', volume: SFX.hero},
	{at: S.cta + 80, name: 'ui-click', volume: SFX.textIn},
];
